...
const Enroll = () => {
  ...
  const handleEnrollment = async () => {
    try {
      ...
      const email = user?.email || "chelluboinarohit@gmail.com";

      console.log("📤 Sending email:", email);

      const res = await fetch("http://127.0.0.1:8000/api/enroll/", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          email: email,
          items: [{ title: course.title }],

          // 🔥 IMPORTANT FIX
          amount: paymentAmount,         // what user pays now
          total_fee: course.price,       // FULL course price

          enrollment_type: paymentType,
          batch_date: date || "10 April 2026",
          payment_method: "upi",
      ...
      // ✅ STORE ENROLLMENT ID
      localStorage.setItem("enrollment_id", data.data.id);
